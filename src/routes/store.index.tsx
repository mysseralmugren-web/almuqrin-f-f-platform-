...
function StoreHome(){
 ...
 return <main className="relative overflow-hidden bg-[#071a2b] text-white">
  ...
  <section aria-label="معرض المقرن التفاعلي" className="relative z-10 h-[100svh] min-h-[640px] snap-y snap-mandatory overflow-y-auto overscroll-contain scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
   {(scenes.length?scenes:[undefined,undefined,undefined,undefined]).map((p,i)=>{
    const bg=imageOf(p);const labels=["المعيشة والمجالس","المكاتب","غرف النوم","التفصيل الخاص"];const fallbackTitles=["أثاث يليق بمساحتك","مساحات عمل أكثر حضورًا","غرفة تُصنع حول أسلوبك","فكرتك تبدأ من هنا"];
    return <article key={p?.product_id??`scene-${i}`} className="relative isolate flex min-h-[100svh] snap-start items-end overflow-hidden border-b border-white/10 px-6 pb-16 pt-28 sm:items-center sm:pb-0 lg:px-10">
      {bg?<img src={bg} alt={p?.name_ar??""} className="absolute inset-0 -z-30 h-full w-full object-cover transition duration-[1400ms] ease-out hover:scale-[1.025]"/>:<div className="absolute inset-0 -z-30 bg-[radial-gradient(circle_at_20%_20%,rgba(197,203,209,.18),transparent_35%),linear-gradient(135deg,#0b2236,#061521)]"/>}
      <div className="absolute inset-0 -z-20 bg-[linear-gradient(90deg,rgba(5,15,25,.18)_0%,rgba(5,15,25,.46)_48%,rgba(5,15,25,.94)_100%)]"/>
      <div className="absolute inset-x-0 bottom-0 -z-10 h-1/2 bg-gradient-to-t from-[#061521]/80 to-transparent"/>
      <div className="mx-auto w-full max-w-7xl">
       <div className="max-w-3xl">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/25 px-4 py-2 text-xs text-white/75 backdrop-blur"><Sparkles className="h-4 w-4"/>{labels[i]}</div>
        {i===0?<h1 className="text-5xl font-black leading-[1.04] tracking-tight sm:text-7xl lg:text-8xl">{p?.name_ar??fallbackTitles[i]}</h1>:<h2 className="text-5xl font-black leading-[1.04] tracking-tight sm:text-7xl lg:text-8xl">{p?.name_ar??fallbackTitles[i]}</h2>}
        <p className="mt-5 max-w-2xl text-sm leading-8 text-white/65 sm:text-base">{p?.description??"استكشف مجموعة مصنع المقرن بطريقة بصرية غامرة، ثم انتقل مباشرة إلى تفاصيل الموديل والتخصيص وطلب السعر."}</p>
        <div className="mt-8 flex flex-wrap gap-3">{p?<Link to="/store/$slug" params={{slug:p.public_slug}} className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-black text-black transition hover:-translate-y-0.5 hover:bg-[#c5cbd1]">استكشف الموديل <ChevronLeft className="h-4 w-4"/></Link>:<a href="#models" className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-black text-black">تصفح الموديلات <ArrowDownLeft className="h-4 w-4"/></a>}<a href="#models" className="rounded-full border border-white/20 bg-black/20 px-6 py-3 text-sm font-bold text-white/75 backdrop-blur transition hover:bg-white/10 hover:text-white">كل المنتجات</a></div>
       </div>
      </div>
      <div className="absolute bottom-7 left-6 flex items-center gap-3 text-[11px] text-white/55 sm:left-10"><span className="text-lg font-black text-white">0{i+1}</span><span className="h-px w-14 bg-white/35"/><span>{p?.category_name_ar??labels[i]}</span></div>
      {i<3&&<div className="absolute bottom-7 right-6 hidden items-center gap-2 text-[11px] text-white/45 sm:flex sm:right-10"><ArrowDownLeft className="h-4 w-4"/> مرّر للمشهد التالي</div>}
    </article>
   })}
  </section>

  <section id="search" className="relative z-10 mx-auto max-w-7xl px-6 py-8">
   <div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-bold tracking-[.18em] text-[#b9bec6]">ALMUQRIN SHOWROOM</div><h2 className="mt-2 text-3xl font-black sm:text-4xl">اختر القطعة وابدأ التخصيص</h2></div><a href="#custom" className="text-xs font-bold text-white/50 hover:text-white">أثاث حسب الطلب ←</a></div>
...
function ProductCard({product,featured,favorite,onFavorite}:{product:Product;featured:boolean;favorite:boolean;onFavorite:()=>void}){
 const image=imageOf(product);const has3d=product.media?.some(m=>m.type==="glb");
