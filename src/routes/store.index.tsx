...
function StoreHome(){
 ...
 return <main className="relative overflow-hidden bg-[#090b0e] text-white">
  <div className="pointer-events-none fixed inset-0 -z-0"><div className="absolute -right-40 -top-40 h-[520px] w-[520px] rounded-full bg-[#9d7440]/10 blur-3xl"/><div className="absolute -left-40 top-[42%] h-[460px] w-[460px] rounded-full bg-slate-500/5 blur-3xl"/></div>
  ...
  <section className="relative isolate overflow-hidden border-b border-white/10">
   ...
   <div className="mx-auto grid min-h-[640px] max-w-7xl items-center gap-6 px-6 py-14 lg:grid-cols-[1.08fr_.92fr] lg:py-16">
    <div className="relative z-10">
     <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#c8a46a]/25 bg-[#c8a46a]/10 px-4 py-2 text-xs text-[#d9bb88]"><Sparkles className="h-4 w-4"/> Showroom تفاعلي · تصنيع محلي</div>
     ...
     <div className="mt-8 flex flex-wrap gap-3"><a href="#models" className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-black text-black transition hover:-translate-y-0.5 hover:bg-[#d8b36b]">اكتشف الموديلات <ArrowDownLeft className="h-4 w-4"/></a><a href="#custom" className="rounded-full border border-white/20 bg-white/[0.03] px-6 py-3 text-sm font-bold text-white/70 transition hover:border-[#c8a46a]/50 hover:text-white">تفصيل حسب الطلب</a></div>
    ...
    <div className="relative min-h-[360px] lg:min-h-[540px]"><AtelierScene image={heroImage}/><div className="absolute bottom-5 right-5 hidden items-center gap-3 rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-xs text-white/60 backdrop-blur sm:flex"><Move3D className="h-5 w-5 text-[#d8b36b]"/><span>تجربة المنتج<br/><b className="text-white">قبل اعتماد الطلب</b></span></div></div>
   </div>
  </section>

  <section id="search" className="relative z-10 mx-auto max-w-7xl px-6 py-8">
   <form onSubmit={search} className="flex flex-col gap-3 rounded-[24px] border border-white/10 bg-white/[0.035] p-3 backdrop-blur md:flex-row"><div className="flex flex-1 items-center gap-3 px-3"><Search className="h-5 w-5 text-[#c8a46a]"/><input name="q" defaultValue={q} className="w-full bg-transparent py-3 text-sm text-white outline-none placeholder:text-white/25" placeholder="مثال: مكتب مدير أسود 240 سم مع أدراج جانبية"/></div><button className="rounded-2xl bg-[#c8a46a] px-7 py-3 text-sm font-black text-black transition hover:bg-[#d8b36b]">بحث ذكي في الموديلات</button></form>
   ...
   <div id="categories" className="mt-4 flex gap-2 overflow-x-auto pb-2">{categories.map(([slug,name])=><button key={slug} type="button" aria-pressed={category===slug} onClick={()=>chooseCategory(slug)} className={`whitespace-nowrap rounded-full border px-4 py-2 text-xs font-bold transition ${category===slug?"border-[#c8a46a] bg-[#c8a46a] text-black":"border-white/10 bg-white/[0.025] text-white/50 hover:border-white/20 hover:text-white"}`}>{name}</button>)}</div>
  </section>

  <section id="models" className="relative z-10 mx-auto max-w-7xl px-6 py-14">
   <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-bold tracking-[.18em] text-[#c8a46a]">ALMUQRIN COLLECTION</div><h2 className="mt-2 text-3xl font-black">مختارات المتجر</h2></div><div className="text-xs text-white/30">افتح المنتج للتفاصيل والتخصيص</div></div>
   {loading?<div className="mt-8 text-sm text-white/40">جارٍ تحميل موديلات المصنع…</div>:products.length?<div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{products.map((p,i)=><ProductCard key={p.product_id} product={p} featured={i===0&&!category&&!q} favorite={favorites.includes(p.product_id)} onFavorite={()=>toggleFavorite(p.product_id)}/>)}</div>:<div className="mt-8 rounded-[30px] border border-dashed border-white/15 bg-white/[0.02] p-12 text-center"><Wrench className="mx-auto h-10 w-10 text-[#c8a46a]"/><h3 className="mt-4 text-xl font-black">لا توجد موديلات منشورة مطابقة</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-7 text-white/35">المتجر لا يعرض أي منتج قبل اكتمال التصنيع، نجاح الجودة، واعتماد السعر والنشر من مدير المنصة.</p></div>}
  </section>

  {mostProduced.length>0&&<section className="relative z-10 mx-auto max-w-7xl px-6 py-10"><div className="rounded-[34px] border border-white/10 bg-[#0f1216] p-8 md:p-12"><div className="text-xs font-bold tracking-[.16em] text-[#c8a46a]">MOST PRODUCED</div><h2 className="mt-2 text-3xl font-black">موديلات أثبتت حضورها في المصنع</h2><div className="mt-8 grid gap-4 md:grid-cols-4">{mostProduced.map((p,i)=><Link key={p.product_id} to="/store/$slug" params={{slug:p.public_slug}} className="rounded-2xl border border-white/10 bg-white/[0.025] p-5 transition hover:-translate-y-1 hover:bg-white/[0.05]"><div className="text-3xl font-black text-[#c8a46a]">0{i+1}</div><div className="mt-4 font-black">{p.name_ar}</div><div className="mt-1 text-xs text-white/35">تم تصنيعه {p.production_history_count} مرة</div></Link>)}</div></div></section>}

  <section id="custom" className="relative z-10 mx-auto max-w-7xl px-6 py-14"><div className="overflow-hidden rounded-[38px] border border-[#c8a46a]/20 bg-[linear-gradient(135deg,#15120e,#0e1115)] p-8 md:p-14"><div className="grid gap-8 lg:grid-cols-[1fr_.65fr] lg:items-end"><div className="max-w-3xl"><div className="flex items-center gap-2 text-xs font-bold text-[#d8b36b]"><Sparkles className="h-4 w-4"/> أثاث حسب الطلب</div><h2 className="mt-3 text-3xl font-black md:text-5xl">عندك صورة أو فكرة؟ نحولها إلى قطعة قابلة للتصنيع.</h2><p className="mt-5 max-w-2xl leading-8 text-white/45">اختر أقرب موديل، أدخل المقاسات والخامات المطلوبة وارفع المرجع. يصل الطلب مباشرة لمنصة المقرن، ويبقى السعر النهائي بانتظار اعتماد المصنع.</p></div><a href="/store#models" className="inline-flex items-center justify-between rounded-2xl bg-white px-5 py-4 text-sm font-black text-black transition hover:bg-[#d8b36b]">ابدأ من موديل <ChevronLeft className="h-4 w-4"/></a></div></div></section>
 </main>
}
...
function ProductCard({product,featured,favorite,onFavorite}:{product:Product;featured:boolean;favorite:boolean;onFavorite:()=>void}){
 ...
 return <article className={`group relative overflow-hidden rounded-[28px] border border-white/10 bg-[#111419] ${featured?"sm:col-span-2 lg:row-span-2":""}`}>
  ...
  <Link to="/store/$slug" params={{slug:product.public_slug}} className="block h-full">
   ...
   <div className={`absolute inset-x-0 bottom-0 p-5 ${featured?"sm:p-7":""}`}><div className="text-[10px] font-semibold tracking-wide text-[#d8b36b]">{product.category_name_ar??"من إنتاج مصنعنا"}</div><h3 className={`mt-2 font-black ${featured?"text-2xl sm:text-3xl":"text-xl"}`}>{product.name_ar}</h3><p className="mt-2 line-clamp-2 max-w-2xl text-xs leading-6 text-white/45">{product.description??"موديل مصنع ومعتمد من مصنع المقرن."}</p><div className="mt-4 flex items-end justify-between gap-4"><div><div className="text-[10px] text-white/30">شامل الضريبة</div><div className="mt-1 text-lg font-black">{sar.format(Number(product.price_with_vat))}</div></div><div className="grid h-11 w-11 place-items-center rounded-full bg-white text-black transition group-hover:bg-[#d8b36b]"><ChevronLeft className="h-4 w-4"/></div></div>{featured&&<div className="mt-5 hidden gap-3 border-t border-white/10 pt-4 text-[11px] text-white/40 sm:flex"><span className="flex items-center gap-1"><Check className="h-3.5 w-3.5 text-[#d8b36b]"/> قابل للتخصيص</span><span>صُنّع {product.production_history_count} مرة</span></div>}</div>
  </Link>
 </article>
}
